          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3 sm:gap-5 min-w-0">
              ...
              <div className="flex items-baseline gap-2 min-w-0">
                ...
                >
                  {allTopics
                    .filter((t) => t.slug === topicSlug || (userTier && userTier !== "free") || t.featured || t.created_by === userId)
                    .map((t) => (
                      ...
                      <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {antiStories.length > 0 && (
                          <div className={`border ${sc.anti.border} rounded-lg p-3 ${sc.anti.bgFaint}`}>
                            <div className={`text-xs ${sc.anti.text} uppercase tracking-wider font-medium mb-2`}>{topic.anti_label} is focused on</div>
                            <ul className="space-y-2">
                              {antiStories.map((s, i) => (
                                <li key={i} className={`text-sm text-gray-300 leading-relaxed pl-3 border-l-2 ${sc.anti.border}`}>{s}</li>
                              ))}
                            </ul>
                          </div>
                        )}
                        {proStories.length > 0 && (
                          <div className={`border ${sc.pro.border} rounded-lg p-3 ${sc.pro.bgFaint}`}>
                            <div className={`text-xs ${sc.pro.text} uppercase tracking-wider font-medium mb-2`}>{topic.pro_label} is focused on</div>
                            <ul className="space-y-2">
                              {proStories.map((s, i) => (
                                <li key={i} className={`text-sm text-gray-300 leading-relaxed pl-3 border-l-2 ${sc.pro.border}`}>{s}</li>
                              ))}
                            </ul>
                          </div>
                        )}
